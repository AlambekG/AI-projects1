...
def print_divider(sizeX):
    for x in range(sizeX):
        print('-', end='')


def print_state(sizeX, sizeY, state, board):
...
def check_rules(state, move, rules):
    ll = [functools.reduce((lambda x, y: x and y), [(state[i] == move) for i in r], True) for r in rules]
    return functools.reduce((lambda x, y: x or y), ll, False)
...
def get_user_move(sizeX, sizeY, state, board, prefix):
    while True:
        ...
        if len(a) != 2:
            ...
            move = a[0] + a[1] * sizeX
            if 0 <= a[0] and a[0] < sizeX and 0 <= a[1] and a[1] < sizeY and state[move] == 0 and \
                    board[int(move / sizeX)][
                        move % sizeX] >= 0:
                return move
            else:
                print("Invalid input.")
...
if __name__ == "__main__":
    ...
    print_divider(sizeX)
    print(" START")
    print_state(sizeX, sizeY, state, board)
    ...
    while True:
        if p1file != '-':
            ...
            move1 = get_user_move(sizeX, sizeY, state, board, "Enter Player 1's move: ")
        ...
        if p2file != '-':
            ...
            move2 = get_user_move(sizeX, sizeY, state, board, "Enter Player 2's move: ")
